...
_H_ADAPTIVE = ("Адаптивное управление — управление светофорным объектом путём непрерывной "
               "оптимизации программы регулирования по критерию эффективности в зависимости "
               "от параметров движения. [ГОСТ Р 71096-2023]\n\n"
...
_H_DELAY = "Задержка ТС — потеря времени в движении ТС; средняя задержка на участке дороги. [ГОСТ Р 70044-2022]"

_H_STOP = ("Остановка ТС — преднамеренное прекращение движения ТС на время до 5 минут "
           "(или более при посадке/высадке пассажиров). "
           "[ПДД, Постановление Правительства РФ № 1090]")

_H_INTENSITY = ("Суточная интенсивность — количество ТС, проходящих через поперечное сечение "
                "дороги за 24 часа (суммарно в обоих направлениях). [ГОСТ 32965-2014]")

_H_SPEED = ("Средняя скорость ТС — отношение пройденного пути к общему затраченному времени "
            "(включая остановки). [ГОСТ 32965-2014]")

_H_PUNKT = ("Пункт учёта (стационарный) — место на дороге для определения интенсивности "
            "движения на регулярной основе. [ГОСТ 32965-2014]")

_H_GLONASS = ("Бортовая система (ГЛОНАСС/GPS) — мобильное связное оборудование на ТС "
              "городского пассажирского транспорта для передачи навигационной "
              "и телематической информации. [ГОСТ Р 57187-2016]")

_H_ADMS = "АДМС — автоматическая дорожная метеорологическая станция."

_H_PUNCT = ("Пунктуальность НГПТ — показатель качества транспортного обслуживания: "
            "точность следования расписанию. [Приказ Минтранса России от 30.12.2021 № 482]")

_H_PASSFLOW = ("Пассажиропоток — движение пассажиров в одном направлении на участке сети "
               "за промежуток времени; количество пассажиров, перевезённых за единицу времени. "
               "[ГОСТ 32965-2014]")

_H_MATURITY = ("Уровень зрелости ИТС — оценка агломерации по развитости цифровых "
               "и автоматизированных инструментов управления транспортом. "
               "[Распоряжение Минтранса России от 27.04.2024 № АК-95-р]\n\n"
               "Применяется ТОЛЬКО к подсистеме S (светофорное управление), "
               "только по доле адаптивного управления (S₁.₁).")
