...
# ── Подсказки из глоссария (точные формулировки) ──────────────────
_H_ADAPTIVE = ("Адаптивное управление — управление светофорным объектом или группой светофорных "
               "объектов путём непрерывной оптимизации программы регулирования или сигнального "
               "плана по критерию эффективности в зависимости от параметров движения, определяемых "
               "детекторами транспорта или иными способами. [ГОСТ Р 71096-2023]\n\n"
               "Примечание: в контексте данных расчётов принимается доля светофорных объектов, "
               "установленных в городской агломерации, подключенных к центру управления дорожным "
               "движением, обеспечивающих адаптивное светофорное регулирование, в общем количестве "
               "светофорных объектов, установленных в городской агломерации.")

_H_DISPATCH = ("Диспетчерское управление - Способ управления светофорной сигнализацией "
               "и позициями управляемых знаков оператором. "
               "[Руководство по проектированию и внедрению автоматизированных систем "
               "управления дорожным движением на базе АССУД]\n\n"
               "Примечание: в контексте данных расчётов принимается доля светофорных объектов, "
               "установленных в городской агломерации, подключенных к центру управления дорожным "
               "движением, обеспечивающих диспетчерское светофорное регулирование, в общем количестве "
               "светофорных объектов, установленных в городской агломерации.")

_H_DELAY = ("Задержка ТС - Потеря времени (задержка) в движении транспортных средств» "
            "в таких показателях, как «средняя задержка транспортных средств в движении "
            "на участке дороги». [ГОСТ Р 70044-2022]")

_H_STOP = ("Остановка ТС - Преднамеренное прекращение движения транспортного средства "
           "на время до 5 минут, а также на большее, если это необходимо для посадки "
           "или высадки пассажиров либо загрузки или разгрузки транспортного средства. "
           "[Правила дорожного движения (утверждены Постановлением Правительства РФ № 1090)]")

_H_INTENSITY = ("Суточная интенсивность - Количество транспортных средств, проходящих через "
                "поперечное сечение автомобильной дороги (на участке, у перекрёстка и т. п.) "
                "за одни сутки (24 часа), обычно суммарно в обоих направлениях. [ГОСТ 32965 2014]")

_H_SPEED = ("Средняя скорость ТС - Отношение пройденного пути к общему затраченному времени "
            "(включая остановки). [ГОСТ 32965‑2014]")

_H_PUNKT = ("Пункт учета/ Стационарный пункт учета интенсивности движения - Место на "
            "автомобильной дороге, предназначенное для определения интенсивности движения "
            "на регулярной основе с преимущественным использованием автоматизированного "
            "метода подсчета. [ГОСТ 32965-2014]")

_H_GLONASS = ("Бортовая система (ГЛОНАСС/GPS)/ бортовые навигационно‑связные терминалы, "
              "использующие ГЛОНАСС/GPS - Мобильное связное оборудование, устанавливаемое на "
              "транспортных средствах городского пассажирского транспорта для регулярной передачи "
              "навигационной и телематической информации по сетям подвижной радиосвязи, обмена "
              "голосовыми сообщениями с системой диспетчерского управления. [ГОСТ Р 57187-2016]")

_H_ADMS = "АДМС - автоматическая дорожная метеорологическая станция."

_H_PUNCT = ("Пунктуальность/ пунктуальность НГПТ - Показатель качества транспортного "
            "обслуживания. Он отражает, насколько точно транспортные средства следуют "
            "утверждённому расписанию. [Приказ Минтранса России от 30.12.2021 № 482]")

_H_PASSFLOW = ("Пассажиропоток - Движение пассажиров в одном направлении на определённом "
               "участке транспортной сети за конкретный промежуток времени. Одновременно термин "
               "используют и как количественную характеристику: число пассажиров, проследовавших "
               "по участку или перевезённых транспортом за единицу времени (час, сутки, месяц "
               "и т. д.). [ГОСТ 32965‑2014]")

_H_MATURITY = ("Уровень зрелости/ Уровень зрелости интеллектуальной транспортной системы "
               "городской агломерации - Оценка агломерации насколько развиты в ней цифровые "
               "и автоматизированные инструменты управления транспортом в соответствии с "
               "Распоряжением Минтранса России от 27.04.2024 № АК-95-р.")
...
with st.sidebar:
    ...
    with st.expander("Список сокращений"):
        st.markdown(
            ...
            "**МТП** — мониторинг транспортного потока  \n"
...
if section.startswith("I. "):
    st.header("I. Управление дорожным движением (светофорное управление)")
    st.warning("⚠️ Не применяется на автомобильных дорогах I и II категории")
    ...
    with t1:
        ...
        st.info("Максимум S₁ = 100 % (полное адаптивное и диспетчерское покрытие в совокупности)")
        c = st.number_input("c — всего светофорных объектов, шт.", 1, 100000, 100, key="c_total")
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("**S₁.₁ — адаптивное управление**")
            st.latex(r"S_{1.1}=\dfrac{a}{c}")
            a  = st.number_input("a — светофоров с адаптивным управлением, шт.", 0, c, min(30, c), help=_H_ADAPTIVE)
            s11 = a / c
            st.metric("S₁.₁", f"{s11:.4f}", help=f"{s11*100:.1f} %")
        with col2:
            st.markdown("**S₁.₂ — диспетчерское управление**")
            st.latex(r"S_{1.2}=\dfrac{b}{c}")
            b  = st.number_input("b — светофоров с диспетчерским управлением, шт.", 0, max(0, c - a), min(60, max(0, c - a)), help=_H_DISPATCH)
            s12 = b / c
        ...
        st.metric("🎯 S₁", f"{s1:.2f} %",
                  help=_H_MATURITY)
        st.caption(f"{grade_maturity(s11 * 100)} — по доле адаптивного управления (S₁.₁ = {s11*100:.1f} %)")

    # ── S2 ──
    with t2:
        st.subheader("S₂ — Изменение средней задержки ТС")
        st.latex(r"S_2 = \left(1 - \frac{\tau_{s2}}{\tau_{s1}}\right)\times 100\%")
        st.info("S₂ > 0 % → задержка снизилась (хорошо)  |  S₂ < 0 % → задержка выросла (плохо)")

        mode2 = st.radio("Режим ввода:", ["Ввести τs напрямую", "Рассчитать τs по участкам дороги"], key="mode2")

        if mode2 == "Ввести τs напрямую":
            col1, col2 = st.columns(2)
            ts1 = col1.number_input("τs₁ — задержка предыдущий период, ч/км", 0.0001, 100.0, 0.025, format="%.4f", help=_H_DELAY)
            ts2 = col2.number_input("τs₂ — задержка текущий период, ч/км",    0.0,    100.0, 0.020, format="%.4f", help=_H_DELAY)
        else:
            st.markdown("#### Формула")
        ...
        s2 = (1 - ts2 / ts1) * 100 if ts1 > 0 else 0.0
        st.divider()
        st.metric("🎯 S₂", f"{s2:.2f} %",
                  help="> 0 % — задержка снизилась (хорошо); < 0 % — задержка выросла (плохо).")
        if s2 >= 0:
            st.caption(f"🟢 Задержка снизилась на {s2:.1f} %")
        else:
            st.caption(f"🔴 Задержка выросла на {abs(s2):.1f} %")

    # ── S3 ──
    with t3:
        st.subheader("S₃ — Изменение числа остановок ТС")
        st.latex(r"S_3 = \left(1 - \frac{N_{\text{ост}2}}{N_{\text{ост}1}}\right)\times 100\%")
        st.info("S₃ > 0 % → остановок стало меньше (хорошо)  |  S₃ < 0 % → остановок стало больше (плохо)")
        mode3 = st.radio("Режим:", ["Ввести Nост напрямую", "Рассчитать по формуле Вебстера"], key="mode3")

        if mode3 == "Ввести Nост напрямую":
            col1, col2 = st.columns(2)
            n1 = col1.number_input("Nост₁.ср — остановок предыдущий период", 0.001, 1e6, 2.5, format="%.3f", help=_H_STOP)
            n2 = col2.number_input("Nост₂.ср — остановок текущий период",    0.0,   1e6, 2.0, format="%.3f", help=_H_STOP)
        else:
    ...
    with t4:
        ...
        col1, col2 = st.columns(2)
        vt2 = col1.number_input("Vt₂ — средняя скорость ТС текущий период, км/ч", 0.1, 500.0, 45.0, help=_H_SPEED)
...
# ══════════════════════════════════════════════════════════════════
    ...
    st.info("Z → 100 % — максимальная безопасность  |  Z → 0 % или < 0 % — снижение уровня безопасности дорожного движения")
    ...
    if is_agglom:
        ...
        N     = col2.number_input("N — суточная интенсивность, авт./сут", 1.0, 1e7, 10000.0, help=_H_INTENSITY)
...
# ══════════════════════════════════════════════════════════════════
    ...
    with t1:
        ...
        with col1:
            ...
            a_m  = st.number_input("a — исправных датчиков, шт.", 0, 100000, 45, help=_H_PUNKT)
            b_m  = st.number_input("b — всего датчиков, шт.",     1, 100000, 50, key="b_m11", help=_H_PUNKT)
            m11 = (a_m / b_m) * 100
            st.metric("M₁.₁", f"{m11:.2f} %")
        with col2:
            st.markdown("**M₁.₂ — данные от бортовых систем (ГЛОНАСС/GPS)**")
            st.latex(r"M_{1.2}=\frac{p}{d}\times 100\%")
            p_m = st.number_input("p — суток с почасовой телеметрией", 0, 365, 85, help=_H_GLONASS)
    ...
    with t6:
        ...
        st.metric("🏆 M — Мониторинг транспортного потока", f"{M:.2f} %",
                  help="Вес в ИТСэф: 0,10. ≥ 80 % — хорошо; 50–80 % — удовлетворительно; < 50 % — неудовлетворительно.")
        grade_md(M)
        save_button("M", "M", M)
...
# ══════════════════════════════════════════════════════════════════
    ...
    with t1:
        ...
        col1,col2=st.columns(2)
        a_h=col1.number_input("a — работоспособных АДМС, шт.",1,100000,9,  help=_H_ADMS)
        b_h=col2.number_input("b — всего АДМС, шт.",           1,100000,10, help=_H_ADMS)
...
# ══════════════════════════════════════════════════════════════════
    ...
    with t4:
        ...
        j_w=col1.number_input("j — инцидентов с нормативной ликвидацией последствий",0,1000000,45,
                              help="Своевременность ликвидации последствий выявленных инцидентов определяется в соответствии с внутренними регламентами уполномоченного органа, занимающегося управлением дорожным движением.")
...
# ══════════════════════════════════════════════════════════════════
    ...
    st.header("VI. Эффективность организации движения НГПТ")
    st.warning("⚠️ Применяется только для городских агломераций")
    ...
    with t2:
        ...
        col1,col2=st.columns(2)
        a_p=col1.number_input("a — рейсов по расписанию", 0,10000000,850,  help=_H_PUNCT)
        b_p=col2.number_input("b — всего рейсов НГПТ",    1,10000000,1000, help=_H_PUNCT)
        r2=a_p/b_p
        st.metric("r₂",f"{r2:.4f}")
    ...
    with t3:
        ...
        col1,col2=st.columns(2)
        e2=col1.number_input("e₂ — пассажиропоток текущий период, чел.",0,100000000,95000,  help=_H_PASSFLOW)
        eb=col2.number_input("eб — базовый пассажиропоток, чел.",        1,100000000,100000, help=_H_PASSFLOW)
        if eb >= e2:
    ...
    with t4:
        st.subheader("Итоговый показатель P — НГПТ")
        st.latex(r"P = 0{,}4\,P_1+0{,}4\,P_2+0{,}2\,P_3")
        ...
        st.metric("🏆 P — НГПТ", f"{P:.2f} %",
                  help="Вес в ИТСэф: 0,20. Применяется только для городских агломераций. ≥ 80 % — хорошо; 50–80 % — удовлетворительно.")
        grade_md(P)
        save_button("P","P",P)
...
# ══════════════════════════════════════════════════════════════════
    ...
    st.info("Значения можно ввести вручную, сохранить из разделов I–VI или заполнить тестовыми данными.")
    ...
    with col_radar:
        st.markdown("<small style='color:rgba(49,51,63,0.6);line-height:0'>Диаграмма-паук показывает баллы каждого комплексного показателя (0–100 %). Чем ближе фигура к внешней границе, тем лучше.</small>", unsafe_allow_html=True)
        labels = ['S — Светофорное упр.', 'Z — Безопасность ДД', 'M — МТП',
                  'H — Метеомониторинг', 'W — Видеонаблюдение', 'P — НГПТ']
    ...
    with col_res:
        ...
        sub_names   = ['S<br>Светофорное упр.', 'Z<br>Безопасность ДД', 'M<br>МТП',
                       'H<br>Метеомониторинг', 'W<br>Видеонаблюдение', 'P<br>НГПТ']
