...
with st.sidebar:
    ...
    labels = {'S': 'S (светофоры)', 'Z': 'Z (БДД)', 'M': 'M (мониторинг)',
              'H': 'H (метео)', 'W_vid': 'W (видео)', 'P': 'P (НГПТ)'}
    any_saved = False
    for k, lbl in labels.items():
        if st.session_state[k] is not None:
            st.write(f"**{lbl}:** {st.session_state[k]:.2f}%")
            any_saved = True
    if not any_saved:
...
# ══════════════════════════════════════════════════════════════════
    ...
    with t6:
        ...
        st.metric("🏆 M — Мониторинг транспортного потока", f"{M:.2f} %"); st.caption(grade(M))
        save_button("M", "M", M)
...
# ══════════════════════════════════════════════════════════════════
    ...
    with t7:
        ...
        st.metric("🏆 H — Метеомониторинг",f"{H:.2f} %"); st.caption(grade(H))
        save_button("H","H",H)
...
# ══════════════════════════════════════════════════════════════════
    ...
    with t5:
        ...
        st.metric("🏆 W — Видеонаблюдение и инциденты",f"{W_vid:.2f} %"); st.caption(grade(W_vid))
...
# ══════════════════════════════════════════════════════════════════
    ...
    with t4:
        ...
        st.metric("🏆 P — Общественный транспорт",f"{P:.2f} %"); st.caption(grade(P))
        save_button("P","P",P)
...
# ══════════════════════════════════════════════════════════════════
    ...
    df = pd.DataFrame({
        "Показатель":    ["S (светофоры)", "Z (БДД)", "M (мониторинг)", "H (метео)", "W (видео)", "P (НГПТ)"],
        "Значение, %":   [S, Z, M, H, W_vid, P],
        "Вес":           [0.2, 0.2, 0.1, 0.1, 0.2, 0.2],
        "Вклад, %":      [0.2*S, 0.2*Z, 0.1*M, 0.1*H, 0.2*W_vid, 0.2*P],
    })
    st.dataframe(
        df.style.format({"Значение, %": "{:.2f}", "Вклад, %": "{:.2f}"}),
