...
if section.startswith("I. "):
    ...
    with t1:
        ...
        with col1:
            ...
            a  = st.number_input("a — светофоров с адаптивным управлением, шт.", 0, c, min(30, c))
            s11 = a / c
            st.metric("S₁.₁", f"{s11:.4f}", help=f"{s11*100:.1f} %")
        with col2:
            st.markdown("**S₁.₂ — диспетчерское управление**")
            st.latex(r"S_{1.2}=\dfrac{b}{c}")
            b  = st.number_input("b — светофоров с диспетчерским управлением, шт.", 0, max(0, c - a), min(60, max(0, c - a)))
            s12 = b / c
